...
function VideoCaptionInner({ active }: { active: boolean }) {
  ...
  const pickFolder = async () => {
    ...
    // 전 플랫폼 네이티브 폴더 피커. Windows는 한때 "파일이 보이는" 파일
    // 다이얼로그로 바꿨었지만(v1.3.0, FOS_PICKFOLDERS가 파일을 숨기는 문제 대응),
    // 파일을 하나 골라야만 '열기'가 동작해 폴더만 고르고 열기를 누르면 OS가
    // 무반응 — 실사용 진행 불가로 보고돼 폴더 피커로 되돌림(2026-07-23).
    const picked = await open({ directory: true, title: "영상 폴더 선택 — 폴더 전체 처리" });
    ...
    <div style={{ ...consoleStyles.panel, display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <section style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <p style={{ margin: 0, fontSize: 12, opacity: 0.6 }}>
          &apos;폴더 선택…&apos;에서 폴더를 고르면 그 안의 모든 영상(하위 폴더 포함)을
          일괄 처리합니다. Windows 폴더 선택 창에는 파일이 표시되지 않지만 폴더를
          고르면 바로 진행됩니다. 특정 파일만 처리하려면 &apos;로컬 파일 선택…&apos;을 쓰세요.
        </p>
        {batchStatus ? (
