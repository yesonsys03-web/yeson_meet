...
//! `RunEvent::Exit` handler in `lib.rs` never runs, so the spawned
//! `yeson-sidecar` (and its `yeson-mac-audio-helper` / `yeson-win-audio-helper`
//! child) survive as orphans from the previous app instance. Those orphans keep
//! their WebSocket session open on the server, which makes the server believe a
//! meeting is still "live" — blocking the next "Go Live" attempt. A shutdown
//! handler now lets the sidecar self-exit cleanly, but Ctrl+C / crashes bypass
//! it — so we want a definitive sweep at startup.
//!
//! This runs ONCE at app startup, BEFORE the operator can start the sidecar.
//! At that point there is no legitimate child of ours yet, so any RUNNING
//! process whose executable matches our EXACT absolute vendored binary path is a
//! leftover from a prior run and is reaped. Matching on the absolute path (not
//! the bare name) means an unrelated `python`/helper elsewhere on the machine is
//! never touched.
//!
...
//! `sidecar` (Unix SIGTERM→grace→SIGKILL by pgid; Windows
...
/// Sweep leftover yeson-sidecar + audio-helper processes from a prior app instance.
/// Call once, early in app startup. Never panics; logs what it reaps via the
/// provided sink.
pub fn reap_orphans(mut log: impl FnMut(&str)) {
    let self_pid = std::process::id();
    if let Some(path) = crate::sidecar::locate_bundled_sidecar() {
        reap_binary(&path, self_pid, "yeson-sidecar", &mut log);
    }
    if let Some(path) = crate::sidecar::locate_bundled_native_helper() {
        reap_binary(&path, self_pid, "audio-helper", &mut log);
    }
}
...
mod tests {
    ...
    fn missing_binary_path_is_a_clean_noop() {
        ...
        reap_binary(
            Path::new("/definitely/not/a/real/vendored/yeson-sidecar"),
            std::process::id(),
            "yeson-sidecar",
