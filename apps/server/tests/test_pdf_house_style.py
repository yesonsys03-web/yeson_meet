...
CASES = [
    ...
    # FL104_Orev 실측 추가분(2026-08-03, 113페이지 전수 대조) — 사람 쪽
    # 예외 0건: 파티악어 167/0, 히피여자 5/0, 이전 씬 위로 재생 19/0,
    # 목욕소금+입욕제 3/0.
    ("파티 게이터:자, 어디까지 얘기했더라?", "파티악어:자, 어디까지 얘기했더라?"),
    ("파티게이터가 웃는다", "파티악어가 웃는다"),
    ("파티 게이터 맨이 벤치에 앉는다", "파티악어맨이 벤치에 앉는다"),
    ("파티 게이터 남자 역", "파티악어맨 역"),
    ("파티게이터맨이 웃는다", "파티악어맨이 웃는다"),
    ("히피 여성", "히피여자"),
    ("히피여성", "히피여자"),
    ("히피 여자", "히피여자"),
    ("이전 샷 위로 재생 싸이클 1/5", "이전 씬 위로 재생 싸이클 1/5"),
    ("배스 솔트 사세요", "목욕소금 사세요"),
    ("배스솔트", "목욕소금"),
]


def test_house_style_party_gator_man_rules_run_first():
    """순서 계약: `파티 게이터`(짧은 규칙)가 먼저 돌면 사람이 쓰지 않는
    띄어쓴 `파티악어 맨`이 남는다 — 사람은 언제나 붙여 쓴 `파티악어맨`이다."""
    assert apply_house_style("파티 게이터 맨") == "파티악어맨"
    assert "파티악어 맨" not in apply_house_style("파티 게이터 맨이 걷는다")


def test_house_style_shot_rule_needs_the_full_phrase():
    """`샷` 단독은 정당한 촬영 용어라 덮지 않는다 — `이전 샷 위로 재생`
    전체 구절일 때만 `씬`으로 고친다(실측: LLM이 19/19 같은 구절로 낸다)."""
    assert apply_house_style("와이드 샷으로 간다") == "와이드 샷으로 간다"
    assert apply_house_style("이전 샷") == "이전 샷"
