...
# 단어 경계 조건을 추가했다 — 앞에 단어 문자가 오면 "여는" 따옴표가
# 아니고(예: "크'" 의 "'"), 뒤에 단어 문자가 오면 "닫는" 따옴표가
# 아니다(예: "'s"의 "'"). 이러면 두 소유격 사이 오폭 3종(리뷰 재현 케이스,
# 연호 표기 "'56년 행크's 트럭" 포함)이 전부 불발하고, 정상 인용
# "행크가 'Owner' 표시를 봤다"는 그대로 변환된다(양쪽 다 공백이 인접해
# 경계 조건을 통과).
_QUOTE_SPAN_RE = re.compile(r"(?<!\w)'([^']{1,40})'(?!\w)")
...
# 뺀 `[^\S\n]+`로 좁혀, 콜론 바로 뒤가 개행뿐이면(공백 없음) 건드리지
# 않는다.
