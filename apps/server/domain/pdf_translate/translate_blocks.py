...
# 전용으로 좁혔다 — 여는 쪽은 `[A-Za-z0-9]`(닫는 쪽엔 필요 없는 숫자도
# 포함, 대칭 요구 없음), 닫는 쪽은 `[A-Za-z]`만. 이러면 라운드 1이 잡은
# 오폭 3종(소유격 두 개, 연호+소유격 조합 "'56년 행크's 트럭")은 그대로
# 막히면서(한글은 이 클래스에 없으니 그 오폭 사례들이 막히는 이유는 다른
# 조건 — 닫는 쪽 바로 뒤가 영문 's'라 라틴 경계에 걸림 — 실제 검증
# 완료), 조사 붙은 정상 케이스는 전부 통과한다(코퍼스 재실측: 34건 중
# 33건 변환, 나머지 1건은 원래도 스팬 형태가 아님 — 아래 실측 참고).
_QUOTE_SPAN_RE = re.compile(r"(?<![A-Za-z0-9])'([^']{1,40})'(?![A-Za-z])")
