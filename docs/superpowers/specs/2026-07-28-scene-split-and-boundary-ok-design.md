...
- 앞 구간 = `{...cur, end_ms: cutMs, label: <원래이름>_cut}`, 뒤 구간 = `{...cur, start_ms: cutMs}`(원래 이름 유지).
- 무효면 `segs`를 그대로 돌려준다(변화 없음): `i` 범위 밖 · `k <= 1` · `cutMs <= start_ms` · `cutMs >= end_ms`.

**두 줄이 같은 이름이면 안 된다**(실기 피드백 2026-07-28): 목록에서 어느 쪽을 고쳐야 할지 알 수 없고, 익스포트 파일명에도 dedupe 접미사가 붙어 헷갈린다. 앞 구간에 `_cut` 임시 이름을 붙여 **아직 이름을 안 정한 쪽이 한눈에 보이게** 한다. 이미 쓰이는 이름이면 숫자를 늘린다(`_cut2`, `_cut3` — 같은 씬을 여러 번 나눠도 겹치지 않게).

진짜 이름은 이 함수가 정하지 않는다. OCR 제안은 비동기라 분할과 시점이 다르고, 이름 변경은 이미 `renameSegment`가 한다 — 한 함수에 두 책임을 넣지 않는다. 못 읽으면 `_cut`이 남아 사용자가 그 줄만 고치면 된다.
...
- 두 구간의 이름이 처음부터 다르므로(`_cut`) 목록의 **"인접 중복 병합"이 방금 나눈 것을 도로 합칠 위험이 없다.** 같은 이름으로 두면 그 버튼 카운트가 올라가 실수로 되돌릴 수 있었다.
