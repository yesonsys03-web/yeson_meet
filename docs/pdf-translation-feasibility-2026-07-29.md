...
## ⚠ 릴리스 노트 필수 고지 (기존 데이터가 지워지는 동작 변경)

- **PDF 작업 자동 정리가 켜집니다.** `prune_old_pdf_jobs`(`RETENTION_KEEP=10`)가
  **서버 기동 시마다** 그리고 새 작업 생성 직후 실행되어, 최근 10건만 남기고
  오래된 PDF 작업의 **DB 행과 작업 폴더(원본+번역본)를 삭제**합니다.
- 따라서 **이미 10건을 넘게 쌓아둔 사용자는 이 버전으로 올린 뒤 첫 기동에서
  오래된 작업이 사라집니다.** 버그가 아니라 기능의 정의지만, 기존 데이터를
  지우는 동작 변경이므로 **릴리스 노트에 반드시 적어야 합니다**(2026-07-30
  전브랜치 리뷰 N-1). 진행 중(queued/extracting/translating/overlaying) 작업은
  아무리 오래돼도 삭제하지 않습니다.
- 근거: 작업 1건이 실측 약 300MB(원본 129MB + 번역본 ~180MB)라 상한이 없으면
  자가호스팅 사용자의 개인 디스크를 무한정 잠식합니다. 영상 리텐션
  (`RETENTION_KEEP=30`)보다 작게 잡은 것도 이 용량 차이 때문입니다.
- 보관이 필요한 산출물은 **번역본을 앱 밖으로 내보낸 뒤** 쌓아 두십시오.
