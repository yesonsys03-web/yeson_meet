...
> - **(2026-06-10) Phase 2b ① 완료(테스터 실증 PASS)**: Job Object 고아정리 — Windows 실기 Task 5b(무음 중 사이드카 강제종료→helper 사라짐) 통과. **(2026-06-10) Phase 2b ② 코드 완료·E2E 대기**: 기본 출력장치 변경 추적(`device_watch.rs`) — 회의 중 출력장치 강등(헤드폰/BT) 시 옛 장치가 무음만 받아 자막이 조용히 끊기던 문제를, **폴링**으로 기본장치 변경 감지 + 헬퍼 **인프로세스 재빌드**(stdout 유지, sidecar/server 무변경)로 새 장치 전환해 해결. Mac `cargo test` + windows-gnu 크로스 `cargo check` 통과, Windows 실기 E2E는 다음 CI 빌드 후 대기. 장치 *제거*는 기존대로 fatal 유지(강등만 대상). **Mac은 ScreenCaptureKit 시스템믹스 탭이라 무관**. 별도 신규: Windows 사이드카 시작 시 cmd 콘솔 깜빡임 수정(`CREATE_NO_WINDOW`)·E2E 대기. 설계: `docs/superpowers/specs|plans/2026-06-10-windows-default-device-watch*.md`.
