...
- [x] 번역문 배치를 원문 **아래 전폭 12pt**로 (2026-07-31, PR#101): 사람 납품본은 아래 여유가 있으면 원문 바로 아래 전폭으로 쓴다(GABE01 1037페이지 실측). `PdfBlock.limit_y`/`limit_x1`(필드 박스 하단·우측)을 `extract()`가 채우고 `place()`가 아래를 먼저 시도, 자리가 없으면 기존 우측 경로. Action Notes의 사람 좌표 대비 거리 172pt → 4pt. 설계: `docs/superpowers/specs/2026-07-31-pdf-translate-below-placement-design.md`
- [x] **3단(다열) 템플릿 + 세로형 지원** (2026-07-31): 사용자 신고 "3단 스토리보드는 번역이 안 된다"의 원인 2개 수정 — ①프로파일이 "페이지당 필드 1세트"를 전제해(`_field_content`가 첫 라벨에서 `break`) 3단 문서에서 뽑은 48블록이 **전부 1열**이었다(FL102_FNL_A 실측, 사람 납품본 기준 필드 59건 중 2·3열 34건 누락) → 앵커 열거(`_field_anchors`)로 열마다 처리, 실질 번역 대상 25 → 60건 ②`detect()`의 가로형 게이트(`w <= h`)가 **612×792 세로형** 익스포트(FL102 `1_PANEL`)를 "지원하지 않는 PDF 포맷"으로 거부 → 게이트 제거(방향은 애초에 4개 포맷을 구분하지 못한다). 부수 수정: 마지막 필드 창을 필드 박스 하단으로 막아 페이지 푸터 유입 차단(3단 77건, 1단 영향 0건) + 우측 폴백의 오른쪽 끝을 `limit_x1`로 조여 열 침범 차단(GABE01 폰트 변화 0·새 클리핑 0으로 실측). 설계: `docs/superpowers/specs/2026-07-31-pdf-translate-multi-panel-design.md`

- [x] **화자 `(씬밖)` 표기 + 브랜드 음역 + 검정 제작 지시어 OCR** (2026-07-31): 3단 산출물을 사람 납품본과 대조해 나온 3건 수정 — ①`(O.S.)`를 생략하던 few-shot이 **소수 사례 기반**이었음이 전수 재측정으로 드러남(GABE01의 (O.S.) 발화는 실질 2개인데 그중 하나는 사람이 `바비(신밖):`으로 표기, FL102는 7건 중 5건 표기) → `화자(씬밖):대사`로 반전 ②스폰서명 `bad choices`를 `잘못된 선택들`로 뜻번역하던 문제 → 브랜드·스폰서명 음역 규칙(단 `잘못된 선택들`은 평범한 대사로도 정당해 KO→KO 강제 치환에는 **의도적으로 넣지 않음**) ③판넬 안 `CAM GUIDE`가 통째로 누락 — OCR은 신뢰도 1.00으로 읽는데 **빨강 비율 문턱**(콜아웃=빨간 글자라는 GABE01 관례)에서 버려지고 있었음 → 문턱을 낮추는 대신 확인된 제작 지시어만 이름으로 통과(그림 속 간판 `REHAB CENTER`는 계속 배제). 하우스 표기 `사이클→싸이클`·`레퍼런스→참고`도 FL102 실측으로 고정(GABE01엔 미등장 = 쇼 간 충돌 없음). 실물 왕복: 한글 주석 75→**86**(사람 88), 패널 콜아웃 15→**26**(사람 29), 결함 0. 설계: `docs/superpowers/specs/2026-07-31-pdf-translate-house-terms-design.md`
