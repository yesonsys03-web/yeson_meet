...
- [x] 전브랜치 리뷰 수정 웨이브 (2026-07-30): 후처리 체인 전체에 한글 가드(번역 실패 폴백의 "영문 불변" 계약을 코드로 강제 — 운영자 용어집 오버라이드로 안전망이 깨지던 경로 차단) + `pdf_job` 리텐션 프루닝(`RETENTION_KEEP=10`, 작업 1건 ≈ 300MB) + 취소·삭제 오류 표면화 + `smoke-server-bundle.ps1` PDF 게이트 + 린트·테스트 품질 정리
