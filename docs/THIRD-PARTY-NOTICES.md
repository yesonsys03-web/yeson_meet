...
### 5.1 PyMuPDF AGPL — 앱 라이선스와의 충돌 (미결)

앱을 독점(All rights reserved)으로 배포하려면 AGPL 링크와 충돌한다. 선택지:

| 방안 | 비고 |
|---|---|
| Artifex 상용 라이선스 취득 | 독점 유지 시 가장 명확. 코드 무변경·품질 위험 0. 비용 발생 |
| PDF 백엔드 교체 (pypdfium2 + pypdf) | **가능함이 실증됐으나 드롭인이 아니다** — §5.1.1 |
| 앱 전체를 AGPL로 공개 | 독점 방침과 배치 |
| 현 상태 유지 + 근거 문서화 | §1의 저볼륨을 근거로. 단 AGPL은 GPL보다 방어 논리가 약하다 |

**결정 전까지 리포에 "All rights reserved" LICENSE를 두는 것은 문서 자체가
사실과 어긋날 수 있다** — 결정 후 함께 정리한다.
